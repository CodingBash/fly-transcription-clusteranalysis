from rnaexp_correlation import find_correlations_simple, find_sanitized_correlations_simple, find_gene, GeneExpressionSetCorrelation
...
def run_sanitized_group_correlation(file, gene_list, corr_limit=0.3, output_file=""):
    output_to_file = len(output_file) > 0
    payload = read_file(file)
    gene_correlation_group = []
    for i in range(len(gene_list)):
        ...
        input_gene = find_gene(gene_list[i], payload[0])
        if input_gene != None:
            correlation_list = find_sanitized_correlations_simple(input_gene, payload[0], corr_limit)
            for k in range(len(gene_list)):
                if i == k:
                    continue
                target_gene = find_gene(gene_list[k], payload[0])
